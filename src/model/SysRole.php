...
class SysRole extends Model
{
    ...
    public static function onAfterWrite($model): void
    {
        ...
        $data = $model->toArray();
        // 【底层优化】添加事务保护：角色-节点关联 + role_path更新确保原子性
        \think\facade\Db::transaction(function () use ($model, $data) {
            if (!empty($data['nodes'])) {
    ...
    public static function onBeforeDelete($model): void
    {
        ...
        $data = $model->toArray();
        $sonCount = $model->whereFindInSet('role_path', $data['id'])->where('id', '<>', $data['id'])->count();
        if (!empty($sonCount)) {
