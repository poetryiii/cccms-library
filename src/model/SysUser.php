...
class SysUser extends Model
{
    ...
    public static function onAfterWrite($model): void
    {
        ...
        $data = $model->toArray();
        // 【底层优化】添加事务保护：确保用户-部门/岗位/角色关联数据写入原子性
        \think\facade\Db::transaction(function () use ($data) {
            ...
            if (isset($data['post_ids'])) {
                ...
                $postIds = is_string($data['post_ids']) ? explode(',', $data['post_ids']) : (array)$data['post_ids'];
                if (!empty(array_filter($postIds))) {
                    $postData = [];
                    foreach ($postIds as $postId) {
                        if ($postId) $postData[] = ['user_id' => $data['id'], 'post_id' => (int)$postId];
                    }
                    if (!empty($postData)) SysUserPost::mk()->saveAll($postData);
                }
            }
            // 【RBAC补全】角色直连关联
            if (isset($data['role_ids'])) {
                SysUserRole::mk()->where('user_id', $data['id'])->delete();
                $roleIds = is_string($data['role_ids']) ? explode(',', $data['role_ids']) : (array)$data['role_ids'];
                if (!empty(array_filter($roleIds))) {
                    $roleData = [];
                    foreach ($roleIds as $roleId) {
                        if ($roleId) $roleData[] = ['user_id' => $data['id'], 'role_id' => (int)$roleId];
                    }
                    if (!empty($roleData)) SysUserRole::mk()->saveAll($roleData);
                }
            }
        });
    }
    ...
    public function getUserMenus(array $userInfo = []): array
    {
        $menus = SysMenu::mk()
            ->where('status', 1)
            ->cache('sys_all_menus', 86400)
            ->column('id,parent_id,name,node,icon,layout_name,target,sort,url,status', 'id');
        $authNodes = array_keys(NodeService::instance()->getAuthNodes());
        foreach ($menus as $mKey => &$mVal) {
            if (isset($menus[$mVal['parent_id']]) && $mVal['parent_id'] !== 0) {
                $menus[$mVal['parent_id']]['url'] = '#';
            }
            $mVal['auth'] = true;
        ...
        $pMenuId = array_column($menus, 'parent_id');
        foreach ($menus as $k => $v) {
            // 判断父级菜单是否存在子级菜单
            if ($v['url'] === '#' && !in_array($v['id'], $pMenuId)) unset($menus[$k]);
        }
        $menus = ArrExtend::toSort($menus, 'sort') ?? [];
        return ArrExtend::toTreeArray($menus, 'id', 'parent_id');
    }
    ...
    /**
     * 【底层优化】设置密码 — MD5 已替换为 password_hash (bcrypt)
     * @param $value
     * @param $data
     * @return string|void
     */
